    ...
    def post(self):
        ...
        if not user:
            security.datastore.create_user(
                ...
                password=hash_password(passwordInput),
...
class InputPatientData(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            db_session.rollback()
            return make_response(
                jsonify(error="Data failed to be added", details=str(e)), 409
            )
...
class GetPatientsData(Resource):
    ...
    @roles_required("user")
    def get(self):
        patients = db_session.query(PatientData).filter(
            PatientData.user_id == current_user.id
        )
        patientList = []
        for patient in patients:
            heartCheck = (
                db_session.query(HeartCheck)
                .filter(HeartCheck.patient_id == patient.id)
                .order_by(HeartCheck.checked_at.desc())
                .first()
            )
            if heartCheck != None:
        ...
        return make_response(jsonify({"data": patientList}), 201)
...
class GetPatientCheckHistory(Resource):
    ...
    def get(self):
        ...
        histories = db_session.query(HeartCheck).filter(
            HeartCheck.patient_id == patient_id
        )
        historyList = []
        if histories:
        ...
        return make_response(jsonify({"data": historyList}), 201)
...
class Preprocessing(Resource):
    ...
    def post(self):
        ...
        patientData = (
            db_session.query(PatientData).filter(PatientData.id == patient_id).first()
        )
        ...
        video_store_path = (
            localstorage + user_directory + patient_directory + f"{self.checked_at}/"
        )
        ...
        if flow_choice == 3:
            ...
            print("Memulai ekstraksi fitur dari video...")
            frames_with_features = processing.video2frame_ayla(video_path = video_link)

            print(f"Jumlah frame yang diproses: {len(frames_with_features)}")

            print("Menyimpan fitur ke file CSV...")
            processing.simpanekstraksifitur_ayla(frames_with_features)

            print("Melakukan klasifikasi frame...")
            y_pred_labels = processing.classify_frames_ayla(frames_with_features, model_path="svm_model_terbaru2.pkl", scaler_path="scaler_terbaru2.pkl")

            print("Menyimpan video hasil deteksi...")
            ...
            res = processing.simpanekstraksifitur_ayla(
                rawImages[0])
            y_pred_labels = processing.classify_frames_ayla(rawImages[0])
            unique_id = str(uuid.uuid1())[:8]
            res = processing.save_result_video_ayla(rawImages[0], y_pred_labels, unique_id)"""
        else:   
            self.frames = processing.video2frames(
            jumlahFrame=self.jumlahFrame, video=video_link)
            ...
            if flow_choice == 1:
                ...
                res = processing.adaptiveThresholding(
                    res, 3, 1, 3, 2
                )  # (blockSize=3, C=1, kernel=3, iterations=2)
            else:
            ...
            res = processing.coLinear(
                R=self.R, CCX=self.CCX, CCY=self.CCY, X1=self.X1, Y1=self.Y1, image=res
            )

            res = processing.triangleEquation(
                R=self.R, CCX=self.CCX, CCY=self.CCY, X1=self.X1, Y1=self.Y1, source=res
            )
            # cv2.imshow("Triangle Equation", res)
            # cv2.waitKey(0)

            GFcoordinates, self.valnorm = processing.GetGoodFeaturesPSAX(
                jumlah=self.jumlah, valnorm=self.valnorm, res=res
            )
            ...
            for i in range(len(rawImages)):
                ...
                if i == 0:
                    for j in range(self.jumlah * 2):
                        x = GFcoordinates[j][0]
                        y = GFcoordinates[j][1]
                        self.goodFeatures[i] = np.append(
                            self.goodFeatures[i], np.array([x, y], dtype=np.float32)
                        )
                    self.goodFeatures[i] = self.goodFeatures[i].reshape(
                        (self.jumlah * 2, 1, 2)
                    )

            self.X1, self.Y1 = processing.findCenterPoint(R=self.R, source=res)

            processing.opticalFlowCalcwithNormalization(
                jumlah=self.jumlah,
                valnorm=self.valnorm,
                lengthDif=self.lengthDif,
                sources=rawImages,
                goodFeatures=self.goodFeatures,
            )

            # Visualisasi tracking
            visualFrames1 = copy.deepcopy(self.frames)
            res = processing.track_visualization(
                jumlah=self.jumlah, images=visualFrames1, goodFeatures=self.goodFeatures
            )
            visualFrames2 = copy.deepcopy(self.frames)
            processing.track_visualization2(
                jumlah=self.jumlah, images=visualFrames2, goodFeatures=self.goodFeatures
            )

            # Feature Extraction
            processing.featureExtractionPSAX(
                jumlah=self.jumlah,
                X1=self.X1,
                Y1=self.Y1,
        ...
        try:
            ...
            return make_response(
                jsonify(
                    {"data": checkResult, "message": "Patient Checked Succesfully"}
                ),
                201,
            )
        except Exception as e:
            db_session.rollback()
            return make_response(
                jsonify(error="Patient failed to checked", details=str(e)), 409
            )
